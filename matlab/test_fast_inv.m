...
if generate 
    ...
    for ik = 1 : length(khv)    
        ...
        %solving the system
        eta    = kh;    
        tic, Fw_mat = Sp + 1i* kh *( Tdiff*Sik + (Spik + eye(n_bd)/2) * (Spik + eye(n_bd)/2) - eye(n_bd)/4)+ ...
                 1i * kh * bsxfun(@times,lambda_imp_orig',S+1i*eta*D*Sik); t1=toc;
        fprintf('time taken in matrix form %d\n',t1);
        %Fw_mat = (T + 1i* eta * Sp + ... %du part
        %    1i * kh * bsxfun(@times,lambda_imp_orig',D+1i*eta*S));%i lambda u part  
        tic, inv_Fw = inv(Fw_mat);  t2= toc;
        fprintf('time taken in matrix inverse %d\n',t2);
        
        xptmp = [xs; ys];
        tic, Fw2_comp = rskelf(Fw_mat,xptmp,30,1e-14); t3=toc;
        fprintf('time taken in fast matrix inverse %d\n',t3);
        %bd_data
        ...
        pot = inv_Fw * bd_data;    
        pot2 = rskelf_sv(Fw2_comp,bd_data);
        umeas(ik).data = (S_tgt + 1i * eta * D_tgt*Sik)*pot;
        data2 = (S_tgt + 1i * eta * D_tgt*Sik)*pot2;
        
        error_data = norm(umeas(ik).data-data2)/norm(data2);
        fprintf('error in applying inverse=%d\n',error_data);
        ...
        sol_a = inv_Fw * rhs_a;
        sol_a2 = rskelf_sv(Fw2_comp,rhs_a);
        utest = (S_tgt + 1i * eta * D_tgt*Sik)*sol_a;
        errs(ik) = norm(utest-uex)/norm(uex);
        fprintf('error=%d\n',errs(ik));
        if mod(ik,15) 
            save('./tmp_t20.mat','umeas','errs','t_lambda',...
            'lambda_imp_orig','khv');
