function [varargout] = resample_curve(srcinfo,h,rl,nh,hcoefs,nout,eps)
%
...
%
  ...
  dydt = -srcinfo(3,:).*srcinfo(5,:)*2*pi/rl;
  dxfft = fft(dxdt)/n;
  dyfft = fft(dydt)/n;
  ...
  par1((2*n+1):(2*n+nhuse)) = hfft/nhuse;
